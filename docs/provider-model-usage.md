...
- `daily` 按日期降序，且**一行一个自然日**：底层 SQL 按 `(provider, model, date, account_id, account_name)` 出行（账号层需要），聚合时再**按日期合并回一行**——即平台层 `models[*].daily` 是跨账号合并后的口径（未勾选「按账号分组」时前端直接读这份，同一通道同一模型同一天不会出现多行），账号层 `accounts[*].models[*].daily` 则在本账号内合并（同一 `account_id` 窗口内改过名也不会重复出行）
- 合并行的派生列**由合并后的原始量重算**，不是对各行再取平均：`avg_duration_ms` = `Σ duration_ms ÷ Σ requests`、`tps` = `Σ 输出 Token ÷ Σ 解码时长`、`cache_hit_ratio` = `Σ cache_read ÷ Σ prompt`。恒等式 `Σ 各日 daily == 该模型 summary == Σ 各账号同模型 summary`（requests / tokens / credit 同口径）
...
- 明细表支持**按账号分组**（控制条最右侧一个勾选框，默认勾选，状态记忆到 `localStorage` 键 `cb_gw_usage_acct`）：勾选开启后，仅**多账号通道**展开账号层，呈现 `平台汇总 → 平台级模型小计 →（账号小计 → 账号内模型小计 → 日明细）`（两级小计都显示），单账号通道维持原有的 `平台汇总 → 模型小计 → 日明细` 三段式不变；该勾选是纯前端展示维度开关（不筛选、不排除某账号），切换不发请求、不消耗限流额度；无任何多账号通道时勾选框置灰但不隐藏
- **未勾选「按账号分组」时，同一通道同一模型的用量合并统计**：日明细一行一个自然日（接口已把该通道该模型当天的各账号行合并），不会出现「同一天多行、每行一个账号、各行平均耗时/速度/命中率互不可比」的情况；合并行的派生列按合并后的原始量重算（见上文「返回结构」），且 `Σ 日明细 == 模型小计 == 平台汇总`
- 任意层级的分组标题行都落在该组内容**之上**：汇总/小计先行，其明细随后（勾选关闭时同样如此，与开启时的表头顺序一致，仅少掉账号层与平台级模型小计下的日期明细）
