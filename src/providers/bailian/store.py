"""Bailian account storage — single-API-key platform.

A Bailian "account" is just the API key itself (no refresh, no expiry, no per-user
quota endpoint we can rely on). We store it as one row in the unified `accounts`
table with provider='bailian', so auth_manager.pick_account / database.record_request
work without any schema change.

Sources:
  1. Admin UI "Import" form: paste the raw API key.
  2. Environment variable CB_BAILIAN_API_KEY (auto-imported on first lookup if no
...
from providers.bailian.constants import CHANNEL_ID, DEFAULT_BASE_URL, ENV_AUTH_DIR, ENV_API_KEY
...
def parse_credentials(body: dict) -> dict:
    ...
    if not isinstance(body, dict):
        raise ValueError("Bailian credentials must be a JSON object")
    raw = (
        body.get("api_key")
        or body.get("apiKey")
        or body.get("key")
    ...
    if not key:
        raise ValueError("Bailian credentials need a non-empty api_key")
    nickname = str(body.get("nickname") or body.get("name") or "bailian").strip() or "bailian"
    base_url = str(body.get("base_url") or DEFAULT_BASE_URL).strip() or DEFAULT_BASE_URL
    return {
        "name": f"bailian-{nickname[:24]}",
        "uid": f"bailian-{key[-8:]}",
        "nickname": nickname,
...
def discover() -> dict:
    """Bailian has no IDE directory. Only an env-var scan."""
    return {
        "channel": CHANNEL_ID,
        "dirs": [],
        "files": [],
...
def ensure_env_account() -> Optional[dict]:
    """If CB_BAILIAN_API_KEY is set and no active Bailian account exists, create one.
