...
export default {props:['token'],setup(p){
  ...
  onMounted(load);return{l,meta,ld,err,open,copied,f,load,fmt,n,tok,money,ms,statusClass,statusText,prev,next,copyErr,pageText,reasoningText,reasoningTitle,I,PROMPT_TIP,CREDIT_TIP}
},template:`
<div>
  ...
  <div class="card" v-else-if="l.length"><table><thead><tr><th>时间</th><th>Key</th><th>账号</th><th>模型</th><th>思考</th><th>Client</th><th>流</th><th :title="PROMPT_TIP">Prompt</th><th>缓存命中</th><th>Completion</th><th>Token</th><th :title="CREDIT_TIP">Credit</th><th>耗时</th><th>状态</th><th></th></tr></thead><tbody>
    <template v-for="x in l" :key="x.id"><tr><td class="mono">{{fmt(x.created_at)}}</td><td>{{x.api_key_name||'-'}}</td><td>{{x.account_name||'-'}}</td><td><code>{{x.model}}</code></td><td><code :title="reasoningTitle(x.reasoning_effort)">{{reasoningText(x.reasoning_effort)}}</code></td><td :title="x.client&&x.client_version?(x.client+' '+x.client_version):(x.client||x.client_version||'-')"><code>{{x.client||'-'}}</code></td><td>{{x.stream?'Stream':'-'}}</td><td>{{tok(x.prompt_tokens)}}</td><td>{{tok(x.cache_read_tokens)}}</td><td>{{tok(x.completion_tokens)}}</td><td>{{tok(x.total_tokens)}}</td><td>{{money(x.credit)}}</td><td>{{ms(x.duration_ms)}}</td><td><span class="badge" :class="statusClass(x)">{{statusText(x)}}</span></td><td><button class="btn s" @click="open=open===x.id?null:x.id">{{open===x.id?'收起':'详情'}}</button></td></tr>
      <tr class="detail-row" v-if="open===x.id"><td colspan="15"><div class="detail-grid"><div><div class="detail-k">Log ID</div><div class="detail-v">{{x.id}}</div></div><div><div class="detail-k">API Key ID</div><div class="detail-v">{{x.api_key_id||'-'}}</div></div><div><div class="detail-k">Account ID</div><div class="detail-v">{{x.account_id||'-'}}</div></div><div><div class="detail-k">HTTP</div><div class="detail-v">{{x.status_code||'-'}}</div></div><div><div class="detail-k">Client</div><div class="detail-v">{{x.client||'-'}}</div></div><div><div class="detail-k">Client 版本</div><div class="detail-v">{{x.client_version||'-'}}</div></div></div><div v-if="x.error_msg"><div class="status-line" style="margin-bottom:8px"><strong style="font-size:12px">错误信息</strong><button class="btn s" @click="copyErr(x)">{{copied?'已复制':'复制错误'}}</button></div><div class="errbox">{{x.error_msg}}</div></div><div class="hint" v-else>该请求没有错误信息。</div></td></tr></template>
