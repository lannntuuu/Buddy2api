...
「用量统计」明细表支持**按账号分组**：控制条最右侧一个勾选框（默认勾选，状态记忆到 `localStorage` 键 `cb_gw_usage_acct`）。开启后**仅多账号通道**展开账号层（平台汇总 / 平台级模型小计 / 账号小计 / 账号内模型小计 / 日明细，两级小计都显示），单账号通道维持原有的 `平台汇总 → 模型小计 → 日明细` 三段式。任意层级的分组标题行都落在该组内容**之上**（汇总/小计先行，明细随后）。该勾选是纯前端展示维度开关（**不筛选、不排除某账号**），切换不发请求、不消耗限流额度。账号归属取自请求日志行的 `account_name` 值拷贝（请求时快照），所以账号被删除后其历史用量仍归属原账号名。

**不勾选时同一通道同一模型合并统计**：日明细一行一个自然日，接口已把该通道该模型当天各账号的行合并，不会再出现「同一天按账号拆成多行、各行平均耗时 / 速度 / 命中率互不可比」；合并行的派生列由合并后的原始量重算（`Σ duration ÷ Σ requests`、`Σ 输出 Token ÷ Σ 解码时长`、`Σ cache_read ÷ Σ prompt`），并保证 `Σ 日明细 == 模型小计 == 平台汇总`。详见 `docs/provider-model-usage.md`。

## 多账号与「锁定账号」

一个通道有多个账号时，默认按**优先级 → 权重 → 已用请求数**自动挑号，同一优先级内还会
「粘住」最近用过的那个号，避免每次随机跳。

在「通道管理」的账号分组头里，可以把通道**锁定**到某个账号：

> **这是「优先用这个号」，不是「只用这个号」。** 锁定号一旦不可用（报错冷却中、被禁用、
> token 过期，或本轮请求已经试过它），请求会**自动换到该通道的其它号**，不会失败、不会卡住。
> 所以不要指望用锁来做流量隔离或精确归因——真实用了哪个号，看「请求日志」页的**账号**列。

锁定号报错后会**临时冷却**，冷却期内自动让位给其它号，到期后**自动恢复**（不需要你手动再锁一次）：

| 报错 | 冷却时长（同账号连续失败时递增） |
|---|---|
| `401` / `403` / `429` | 30 秒起，逐次翻倍，最长 5 分钟 |
| 其它错误（5xx / 网络等） | 5 秒起，逐次翻倍，最长 80 秒 |

⚠️ **特别注意**：`401` / `403`（凭据失效）除了冷却，还会把该账号状态**直接标成 `expired`**，
它会从自动调度里消失。此时需要重新导入/刷新该账号凭据，或手工把状态改回 `active`。

其它要点：

- 锁定是**每通道独立**的：给 workbuddy 锁了 A，不影响 traework。
- 关于重试：**不可重试的错误（如 `400` / `404` / `422`）不会换号**，会直接把错误返回给客户端——
换号也救不了请求本身的错误。流式请求**一旦已经开始输出内容就不再换号**（已发出的内容收不回）。
- 锁定值存在 `src/gateway_settings.json` 的 `manual_account_pin` 键里，**不在数据库**：
备份/迁移数据库**不会**带上它，换机器后需要重新锁。
- 删除账号或通道**不会**清掉它的锁定记录；若之后重建同名通道，旧锁定会**静默生效**。
要彻底清掉，在界面上把该通道的锁定切回「自动」。

完整口径（选号优先级、冷却公式、各通道重试差异）见 `docs/design/account-routing.md`。
...
│  ├─ design/               # account-routing（账号调度/故障转移口径）/ per-model-reasoning-effort 等设计稿
│  ├─ maintenance/          # 维护手册
│  ├─ releases/             # 发布说明
...
- **自定义通道**：在管理页「通道管理」点行内「详情」浮窗的「编辑」可直接新增/编辑/删除任意 OpenAI 兼容平台，零代码、热生效；协议实现统一走 `providers/openai_compat.py` 基类。列表中两组各支持 ≡ 拖拽排序（密钥型组固定在登录型组之后）。新增浮窗仅「通道 ID / 显示名 / Base URL / 创建模式 API Key」为必填（`*` 红色）；**模型白名单可选**，留空默认 `["DeepSeek-V4-Flash"]`（保存后可在「模型配置」页调整或用探活拉取）；**环境变量名可选**，留空自动生成 `CB_<通道ID大写>`（显式值仍须匹配 `^CB_[A-Z0-9_-]+$`，与通道 ID 字符集对齐）。编辑模式留空 API Key 表示不轮换旧 Key。
