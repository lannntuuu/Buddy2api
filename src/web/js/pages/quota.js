...
export default {props:['token','toast'],setup(p){
  ...
  const claimTotals=computed(()=>{const rs=(claim.value&&claim.value.results)||[];const cl=rs.filter(r=>r&&r.claimed);return{total:rs.length,claimed:cl.length,credit:cl.reduce((s,r)=>s+(Number(r.credit)||0),0),already:rs.filter(r=>r&&!r.claimed&&r.already_claimed).length,failed:rs.filter(r=>r&&!r.ok).length}});
  ...
  onMounted(load);return{l,ld,cld,checkins,checkinSummary,checkinLoading,claimingAll,officialRefreshing,pkg,claim,claimTotals,busyKey,withBusy,load,loadCredit,loadCheckins,refreshAllResources,refreshResource,openPackages,refreshPackages,claimOne,claimAll,rows,alerts,creditSum,fmt,credit,age,officialBalance,officialMeta,cacheAge,officialWarn,expireMeta,checkinOf,checkinClass,checkinText,claimText,shortTime,expireText,pkgBadge,chName,I}
},template:`
<div>
  ...
  <div class="ov" v-if="claim" @click.self="claim=null"><div class="modal"><div class="modal-h"><h3>{{claim.title}}</h3><button class="x" @click="claim=null">&times;</button></div><div class="modal-b"><div class="testbox" v-if="claim.results?.length"><div class="row"><span>账号</span><span>{{claimTotals.total}}</span></div><div class="row"><span>本次领取</span><span>{{claimTotals.claimed}} · {{credit(claimTotals.credit)}} credit</span></div><div class="row"><span>今日已领</span><span>{{claimTotals.already}}</span></div><div class="row"><span>失败</span><span>{{claimTotals.failed}}</span></div></div><table class="mini-table" style="margin-top:10px" v-if="claim.results?.length"><thead><tr><th>账号</th><th>结果</th><th>连续</th><th>HTTP</th></tr></thead><tbody><tr v-for="r in claim.results" :key="r.account_id"><td :title="'通道：'+(r.channel||'workbuddy')">{{r.account_name}}</td><td><span class="badge" :class="r.ok?'ok':'err'">{{claimText(r)}}</span></td><td>{{r.streak_days??'-'}}</td><td class="mono">{{r.status_code||'-'}}</td></tr></tbody></table><div class="hint" style="margin-top:8px">这是手动触发的领取动作；不会定时领取，也不会绕过验证或风控。</div></div><div class="modal-f"><button class="btn pri" @click="claim=null">关闭</button></div></div></div>
</div>`};
