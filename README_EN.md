...
- **Docker:** `powershell -ExecutionPolicy Bypass -File .\ops\start-docker-win.ps1`. The script still starts even when there's no WorkBuddy login directory on the host. All seven channels (with Bailian / GMI opt-in) are still in the dropdown when their `CB_GATEWAY_PROVIDERS` flag is set, but use the `python -m src.gateway.server` method above for QClaw / QwenWork. TraeWork's login file isn't DPAPI, so once imported via local `python -m src.gateway.server`, Docker can use the tokens from the database. Trae SOLO doesn't read a local directory — its login loop and tokens live in the database, so it works inside the container too. GMI and Bailian are Web-imported and work inside the container as well.
...
4. Fill in your client:
   ...
   - Model: WorkBuddy `auto`; QClaw `auto`; QwenWork `auto` or `qwork-advanced`; TraeWork `auto` or `qwen-3.7-plus`; Trae SOLO `auto` or `glm-5.2` (`auto` maps to `glm-5.2` on SOLO); GMI or Bailian `auto` or whatever the upstream lists
...
- `git` or `conda` is not a recognized command: the terminal didn't inherit PATH; close it and reopen; Conda users should use Miniconda Prompt from the Start menu instead of plain PowerShell.
- `No module named ...`: dependencies weren't installed into the current Python. First `conda activate buddy2api` (or activate the matching virtualenv), then `python -m pip install -r ops/requirements/base.txt`, then run the gateway again.
- Dependency download is very slow: make sure PyPI is reachable; don't mix several Python installs (system Python, conda env, `.venv` will fight over packages).
- Port 8787 is occupied: stop the old Buddy2api process, then start the new one; or use a different port via `python -m src.gateway.server --port 8788`.
- No accounts in the web UI: not imported yet. Open **Accounts**, pick the right channel and click "Re-detect"; if the login directory is wrong, set `CB_AUTH_DIR` / `CB_QCLAW_AUTH_DIR` / `CB_QWENWORK_AUTH_DIR` and detect again.
- Key creation failed: the create form had no channel selected — since v2.0 every key must be bound to a channel.
- Client 503 `channel_unavailable`: the channel bound to this key has no usable account yet, or all accounts are expired. Go to **Accounts** and run checkin / refresh first.
- Client 403 `key_channel_mismatch`: the model carries another channel's prefix (e.g. `qwenwork/...`) which doesn't match the key's bound channel. Either switch to a key bound to that channel, or use a model id that matches this key's channel.
- Client 400 `unknown_model`: the model doesn't belong to this key's channel. Switch keys, or use an id that channel recognizes (see `/v1/models` for the per-channel whitelist).
...
|---|---|
| `CB_GATEWAY_PROVIDERS` | Which channels to enable, comma-separated. Default `workbuddy,qclaw,qwenwork,traework,traesolo`. GMI and Bailian are opt-in; enable by appending them: `workbuddy,qclaw,qwenwork,traework,traesolo,gmi` or `workbuddy,qclaw,qwenwork,traework,traesolo,bailian`. |
...
**When the checkbox is cleared, the usage of the same channel and the same model is merged outright**: no account layer is expanded, and both the model subtotal and the daily detail are the totals across all of that channel/model's accounts — one row per calendar day (never "several rows for the same day, one per account, with incomparable per-row average duration / speed / hit rate"). The merged row's `avg_duration_ms` / `tps` / cache hit ratio are recomputed from the merged raw sums, and `Σ daily detail == model subtotal == provider summary`.
