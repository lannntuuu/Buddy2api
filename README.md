...
「用量统计」明细表支持**按账号分组**：控制条最右侧一个勾选框（默认勾选，状态记忆到 `localStorage` 键 `cb_gw_usage_acct`）。开启后**仅多账号通道**展开账号层（平台汇总 / 账号小计 / 账号内模型小计 / 日明细，两级小计都显示），单账号通道维持原有的 `平台汇总 → 模型小计 → 日明细` 三段式。该勾选是纯前端展示维度开关（**不筛选、不排除某账号**），切换不发请求、不消耗限流额度。账号归属取自请求日志行的 `account_name` 值拷贝（请求时快照），所以账号被删除后其历史用量仍归属原账号名。
