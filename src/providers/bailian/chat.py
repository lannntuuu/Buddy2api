...
from providers.bailian import store
from providers.bailian.constants import (
    ALIASES,
    CHANNEL_ID,
    DEFAULT_BASE_URL,
    DEFAULT_MODEL,
...
logger = logging.getLogger("bailian.chat")
...
def accepts_model(inner: str) -> bool:
    ...
    Whitelist/aliases honour admin customisation (bailian.models / bailian.aliases
...
async def refresh_model_ids(force: bool = False) -> list[str]:
    ...
    try:
        ...
        logger.warning("bailian /v1/models refresh failed: %s", exc)
    _models_cache.update({"fetched_at": now, "ids": list(STATIC_MODELS)})
    return list(STATIC_MODELS)
...
async def _pick_account(exclude_ids: set[int] | None = None):
    """Pick the single active Bailian account. Single-key platform → no rotation."""
...
async def chat_completions(payload: dict, api_key_info: dict | None) -> tuple:
    ...
    if not account:
        return "error", (503, {
            "error": {
                "message": "No active Bailian account configured (set CB_BAILIAN_API_KEY or import via admin UI)",
