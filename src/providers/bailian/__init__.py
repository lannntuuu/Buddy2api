"""阿里百炼 Bailian (MaaS dedicated instance) provider — OpenAI-compatible.
...
from providers.bailian import chat as _chat
from providers.bailian import quota, store
from providers.bailian.constants import CHANNEL_ID, DISPLAY_NAME
...
class BailianProvider:
    ...
    def fetch_model_rates(self) -> list[dict]:
        """Bailian 上游 /v1/models 不回报倍率，仅返回生效白名单（rate=None, official=False）。"""
        return [
    ...
    async def refresh(self, account: dict) -> dict:
        ...
        return {"status": "noop", "message": "bailian uses a static API key; rotate by re-importing"}
